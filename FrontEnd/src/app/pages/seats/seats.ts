...
export class Seats implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seatsService.getSeatId(this.show_id, this.screening_id).subscribe((data) => {
      this.customreservedSeats = data;
    });
  }
    ...
    calculateTotal(): number {
      let seatsTotal = this.selectedSeats.length * 100; 
    ...
    isSeatReserved(seat_id: number): boolean {
      return this.customreservedSeats.some((res) => res.seat_id === seat_id);
    }
